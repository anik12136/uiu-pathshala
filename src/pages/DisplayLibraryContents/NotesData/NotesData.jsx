...
const NotesData = ({ subject }) => {
  ...
  console.log(books);
  const booksLoader = () => {
    fetch(`https://server-uiu-pathshala.vercel.app/notes?courseName=${subject.courseName}`)
      .then((res) => res.json())
  ...
  const bookFormDataHandler = (e) => {
    ...
    formData.append("noteName", e.target.noteName.value);
    ...
    <div>
      ...
      >
        ...
        <div className="w-96 lg:w-[700px] p-2 lg:p-4 bg-white rounded-md ">
          ...
          <div>
            <h1 className="font-bold text-xl mb-3">Add a New Note</h1>
            <form onSubmit={bookFormDataHandler}>
              <label className="text-sm text-gray-700 ms-1">Note Name</label>{" "}
              <br></br>
              <input
                type="text"
                name="courseName"
                ...
                readOnly
              />
              <label className="text-sm text-gray-700 ms-1">Note Code</label>{" "}
              <br></br>
              <input
                type="text"
                name="courseCode"
              ...
              <label className="text-sm text-gray-700 ms-1">
                Note Name <span className="text-red-700">*</span>
              </label>{" "}
              <br></br>
              <input
                type="text"
      ...
      <div>
        ...
        >
          <IoIosAddCircleOutline className="text-xl lg:text-2xl" /> New Note
        </button>
      </div>
      <table className="table-auto w-full my-2 lg:my-4">
        <tbody>
          <tr className="border-b border-gray-300">
            <th></th>
            <th className="text-start py-2">Note</th>
